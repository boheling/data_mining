...
sub add_instances {
    ...
    push @{ $self->{labels} }, $params{label}
        unless exists $self->{stat_labels}->{ $params{label} };
    ...
    foreach my $a ( keys( %{ $self->{stat_attributes} } ) ) {
        ...
        $self->{stat_attributes}{$a}{$attval}{ $params{label} }
            += $params{cases};
    }
}
...
sub train {
    ...
    foreach my $label ( keys( %{ $self->{stat_labels} } ) ) {
        $m->{labelprob}{$label}
            = $self->{stat_labels}{$label} / $self->{numof_instances};
    }
            ...
            foreach my $attval ( keys( %{ $self->{stat_attributes}{$att} } ) )
            {
                next
                    ...
                    exists( $self->{stat_attributes}{$att}{$attval}{$label} )
                        and $self->{stat_attributes}{$att}{$attval}{$label}
                        > 0;
                push @attvals, $attval;
                $m->{condprob}{$att}{$attval} = {}
                    unless exists( $m->{condprob}{$att}{$attval} );
                $m->{condprob}{$att}{$attval}{$label}
                    = $self->{stat_attributes}{$att}{$attval}{$label};
                $m->{condprobe}{$att}{$attval} = {}
                    unless exists( $m->{condprob}{$att}{$attval} );
                $m->{condprobe}{$att}{$attval}{$label}
                    = $self->{stat_attributes}{$att}{$attval}{$label};
                $total += $m->{condprob}{$att}{$attval}{$label};
            }
            if ( exists( $self->{smoothing}{$att} )
                and $self->{smoothing}{$att} =~ /^unseen count=/ )
            {
                my $uc = $';
                $uc = 0.5 if $uc <= 0;
    ...
    foreach my $att ( keys %{ $m->{real_attr} } ) {
        ...
        foreach my $attval ( keys %{ $self->{stat_attributes}{$att} } ) {
            foreach my $label (
                keys %{ $self->{stat_attributes}{$att}{$attval} } )
            {
                $m->{real_stat}{$att}{$label}{sum} += $attval
                    * $self->{stat_attributes}{$att}{$attval}{$label};
            ...
            {
                next
                    if !defined( $m->{real_stat}{$att}{$label}{count} )
...
sub predict {
    my ( $self, %params ) = @_;
    my $newattrs = $params{attributes}
        or die "Missing 'attributes' parameter for predict()";
    ...
    foreach my $att ( keys( %{$newattrs} ) ) {
        ...
        die
            unless exists( $self->{stat_attributes}{$att}{$attval} )
                or exists( $self->{smoothing}{$att} );
        foreach my $label (@labels) {
            if (    exists( $m->{condprob}{$att}{$attval} )
                and exists( $m->{condprob}{$att}{$attval}{$label} )
                and $m->{condprob}{$att}{$attval}{$label} > 0 )
            {
                $scores{$label} *= $m->{condprob}{$att}{$attval}{$label};
            }
            elsif ( exists( $self->{smoothing}{$att} ) ) {
                $scores{$label} *= $m->{condprob}{$att}{'*'}{$label};
            }
            else { $scores{$label} = 0 }
...
sub print_model {
    my $self       = shift;
    my $withcounts = '';
    if ( $#_ > -1 && $_[0] eq 'with counts' ) { shift @_; $withcounts = 1; }
    my $m      = $self->{model};
    my @labels = $self->labels;
    my $r;
        ...
        foreach my $label (@labels) {
            if ( !exists( $m->{real_attr}->{$att} ) ) {
                foreach my $attval (@attvals) {
                    next
                        unless
                        exists( $m->{condprob}{$att}{$attval}{$label} );
                    push @lines,  "$label ";
                    push @lines1, "$attval ";
...
sub _append_lines {
    ...
    @l = map {
        while ( length($_) < $m ) { $_ .= substr( $_, length($_) - 1 ) }
        $_
    } @l;
    return @l;
}
